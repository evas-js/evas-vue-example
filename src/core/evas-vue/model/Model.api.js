...
export function setModelApi(model, api) {
    ...
    model.thowIfNotHasApiRoute = function (action) {
        ...
        if (!this.routes.actions) throw new Error(`${this.entityName} routes actions does not exists`)
        if (!this.routes.actions.includes(action)) {
            throw new Error(`${this.entityName} not has routes action ${action}`)
        }
    }
     ...
     * @returns { Promise }
     */
    model.apiDataFetched = function (response, name) {    
        console.log('apiDataFetched', response?.body)
        const hooked = this.beforeFetched(name, response)
        const entities = this.fetchedDataParser.parse(hooked?.body)
        return [entities, hooked]
    }
     ...
     * @returns { Promise }
     */
    model.callApiRoute = function (action, args) {
        this.thowIfNotHasApiRoute(action)
        // return this.api.call(this.entityName, action, args)
        return this.api.call(this, action, args)
    }
    ...
    model.fetch = function (name, args) {
        ...
        return new Promise((resolve) => {
            this.callApiRoute(name, args).then((response) => {
                console.log('fetch', response?.body)
                resolve(...this.apiDataFetched(response, name))
            })
        })
    }
    ...
    model.beforeFetched = function (name, response) {
        return response
    }
